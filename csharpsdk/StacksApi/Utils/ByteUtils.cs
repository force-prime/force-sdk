...
namespace StacksForce.Utils
{
    ...
    {
        static public string ToHex(this byte[] bytes)
        {
        ...
        static public byte[] UInt32ToByteArray(UInt32 value)
        {
            return Enumerable.Range(0, 4).Select(x => (byte)((value >> ((3 - x) * 8)) % 256)).ToArray();
        }

        static public byte[] UInt64ToByteArray(ulong value)
        {
            return Enumerable.Range(0, 8).Select(x => (byte)((value >> ((7 - x) * 8)) % 256)).ToArray();
        }

        static public byte[] UInt64ToByteArray(ulong value, int byteCount)
        {
