namespace StacksForce.Stacks.ChainTransactions
{
    ...
    {
        static public Transaction DeployContract(Blockchain blockchain, StacksAccount sender, string contractName, string contractCode, uint fee = 0, uint nonce = 0) {
        ...
        public static Transaction ContractCall(Blockchain blockchain, StacksAccount sender, string address, string contract, string method, Clarity.Value[]? arguments, PostCondition[]? postConditions = null, uint fee = 0, uint nonce = 0)
        {
            ...
            var auth = GetBasicAuthorization(sender, fee, nonce);
            var transaction = CreateTransaction(blockchain, auth, payload, postConditions);

            return transaction;
        }

        public static Transaction StxTransfer(Blockchain blockchain, StacksAccount sender, string principal, ulong amount, string? memo = null, uint fee = 0, uint nonce = 0)
        {
        ...
        static private Authorization GetBasicAuthorization(StacksAccount sender, uint fee, uint nonce)
        {
